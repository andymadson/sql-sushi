WITH clover AS (
    SELECT
        ...
        c.payment_method
    FROM {{ source('raw', 'clover_transactions') }} AS c
    LEFT JOIN {{ source('raw', 'master_items') }} AS mi
...
square AS (
    SELECT
        ...
        s.payment_method
    FROM {{ source('raw', 'square_transactions') }} AS s
    LEFT JOIN {{ source('raw', 'master_items') }} AS mi
...
toast AS (
    SELECT
        ...
        t.transaction_ts,
        t.payment_method
    FROM {{ source('raw', 'toast_transactions') }} AS t
    LEFT JOIN {{ source('raw', 'master_items') }} AS mi
